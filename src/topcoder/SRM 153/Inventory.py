...
def do_test(sales, daysAvailable, __expected):
    startTime = time.time()
    instance = Inventory()
    exception = None
    try:
        __result = instance.monthlyOrder(sales, daysAvailable);
    except:
        import traceback
...
def run_tests():
    sys.stdout.write("Inventory (250 Points)\n\n")
    ...
    with open("Inventory.sample", "r") as f:
        while True:
            label = f.readline()
            ...
            daysAvailable = []
            for i in range(0, int(f.readline())):
                daysAvailable.append(int(f.readline().rstrip()))
            daysAvailable = tuple(daysAvailable)
            f.readline()
            ...
            passed += do_test(sales, daysAvailable, __answer)

    sys.stdout.write("\nPassed : %d / %d cases\n" % (passed, cases))

    T = time.time() - 1455655186
    PT, TT = (T / 60.0, 75.0)
