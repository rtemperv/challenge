...
def do_test(child, mother, men, __expected):
    startTime = time.time()
    instance = PaternityTest()
    exception = None
    try:
        __result = instance.possibleFathers(child, mother, men);
    except:
        import traceback
...
def run_tests():
    sys.stdout.write("PaternityTest (300 Points)\n\n")
    ...
    with open("PaternityTest.sample", "r") as f:
        while True:
            label = f.readline()
            if not label.startswith("--"): break

            child = f.readline().rstrip()
            mother = f.readline().rstrip()
            men = []
            for i in range(0, int(f.readline())):
                men.append(f.readline().rstrip())
            men = tuple(men)
            f.readline()
            __answer = []
            for i in range(0, int(f.readline())):
            ...
            passed += do_test(child, mother, men, __answer)

    sys.stdout.write("\nPassed : %d / %d cases\n" % (passed, cases))

    T = time.time() - 1455655118
    PT, TT = (T / 60.0, 75.0)
